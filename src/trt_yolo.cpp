...
bool Model::prepare() {
  ...
  out_boxes_d_ =
      cuda::make_unique<float[]>(getMaxBatchSize() * getMaxDets() * 4);
...
Model::Model(const std::string &engine_path, bool verbose) {
  ...
  runtime_ =
      unique_ptr<nvinfer1::IRuntime>(nvinfer1::createInferRuntime(logger));
  load(engine_path);
  if (!prepare()) {
...
Model::Model(const std::string &onnx_path, const std::string &precision,
             const int max_batch_size, const bool verbose,
  ...
  // Create builder
  auto builder =
      unique_ptr<nvinfer1::IBuilder>(nvinfer1::createInferBuilder(logger));
  if (!builder) {
    std::cerr << "[ERROR] Fail to create builder!!" << std::endl;
    std::exit(1);
  }
  auto config =
      unique_ptr<nvinfer1::IBuilderConfig>(builder->createBuilderConfig());
  if (!config) {
  ...
  auto network =
      unique_ptr<nvinfer1::INetworkDefinition>(builder->createNetworkV2(flag));
  if (!network) {
  ...
  // Build engine
  std::cout << "[INFO] Applying optimizations and building TRT CUDA engine..."
            << std::endl;
...
bool Model::detect(const cv::Mat &img, float *out_scores, float *out_boxes) {
  ...
  CHECK_CUDA_ERROR(cudaMemcpyAsync(out_scores, out_scores_d_.get(),
                                   sizeof(float) * getMaxDets(),
                                   cudaMemcpyDeviceToHost, stream_));
  CHECK_CUDA_ERROR(cudaMemcpyAsync(out_boxes, out_boxes_d_.get(),
                                   sizeof(float) * 4 * getMaxDets(),
                                   cudaMemcpyDeviceToHost, stream_));
  cudaStreamSynchronize(stream_);
  return true;
}
...
int Model::getInputSize() const {
  const auto input_dims = getInputDims();
  return std::accumulate(input_dims.begin(), input_dims.end(), 1,
                         std::multiplies<int>());
}

int Model::getMaxBatchSize() const {
  return engine_->getProfileDimensions(0, 0, nvinfer1::OptProfileSelector::kMAX)
      .d[0];
}
int Model::getMaxDets() const { return engine_->getBindingDimensions(1).d[1]; }

void Model::setOuputHeadNames(std::string &box_head_name,
