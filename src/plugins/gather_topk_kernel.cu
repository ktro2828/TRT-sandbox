...
#include "plugins/common_cuda_helper.hpp"
#include "plugins/gather_topk_kernel.hpp"
#include "plugins/trt_plugin_helper.hpp"

template <typename scalar_t>
__global__ void gather_topk_kernel(const scalar_t* input, const int* indices,
                                   scalar_t* output, int batch, int num_input,
  ...
  CUDA_1D_KERNEL_LOOP(index, batch * num_indices * channel) {
    ...
    const scalar_t value =
        input[b_id * num_input * channel + input_n_id * channel + c_id];
    output[b_id * num_indices * channel + n_id * channel + c_id] = value;
  }
}

template <typename scalar_t>
void gather_topk_impl(const scalar_t* input, const int* indices,
                      const int* dims, int nbDims, const int* indices_dims,
                      int indice_nbDims, scalar_t* output,
                      cudaStream_t stream) {
  int batch = 1;
...
template void gather_topk_impl<float>(const float* input, const int* indices,
                                      const int* dims, int nbDims,
                                      const int* indices_dims,
                                      int indice_nbDims, float* output,
                                      cudaStream_t stream);

template void gather_topk_impl<int32_t>(const int32_t* input,
                                        const int* indices, const int* dims,
                                        int nbDims, const int* indices_dims,
                                        int indice_nbDims, int32_t* output,
                                        cudaStream_t stream);