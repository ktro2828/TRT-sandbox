...
cv::Mat drawOutput(const cv::Mat &img, const float *scores, const float *boxes,
  ...
  for (int i = 0; i < config.num_max_detections; ++i) {
    ...
    cv::rectangle(viz, cv::Point(x1, y1), cv::Point(x2, y2),
...
int main(int argc, char *argv[]) {
  if (argc != 3) {
    std::cerr << "[ERROR] You must specify input path of image and engine!!"
              << std::endl;
    std::exit(1);
  }
  ...
  std::cout << "[INFO] engine: " << engine_path << ", onnx: " << onnx_path
            << std::endl;
  ...
  if (fs::exists(engine_path)) {
    ...
    if (max_batch_size != model_ptr->getMaxBatchSize()) {
      std::cout << "[INFO] Required max batch size " << max_batch_size
                << "does not correspond to Profile max batch size "
                << model_ptr->getMaxBatchSize() << ". Rebuild engine from onnx."
                << std::endl;
      model_ptr.reset(
          new ssd::Model(onnx_path, precision, max_batch_size, verbose));
  ...
  std::unique_ptr<float[]> out_scores = std::make_unique<float[]>(
      model_ptr->getMaxBatchSize() * model_ptr->getMaxDets());
  std::unique_ptr<float[]> out_boxes = std::make_unique<float[]>(
      model_ptr->getMaxBatchSize() * model_ptr->getMaxDets() * 4);
